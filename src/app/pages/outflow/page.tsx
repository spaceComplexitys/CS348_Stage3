...
const Outflow = () => {
  ...
  const fetchTransactions = useCallback(async (outflowValue: string) => {
    setLoading(true);
    try {
      const response = await fetch(`/api/filterOutflow?outflow=${outflowValue}`);
  ...
  // Debounce the API call to handle frequent input changes efficiently
  useEffect(() => {
    const timeout = setTimeout(() => {
      fetchTransactions(searchQuery || '0'); // Default to `outflow=10` if search query is empty
      ...
      <div style={styles.header}>
        ...
        <div style={styles.searchContainer}>
          <input
            ...
            type="text"
            placeholder="Search outflow amount"
            value={searchQuery}
